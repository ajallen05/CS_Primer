...
def palindrome(letters):
    ...
    if not letters[-1].isalpha():
        return palindrome(letters[:-1])

    elif not letters[0].isalpha():
        return palindrome(letters[1:])
...
def iter_gcd(a, b):
    ...
    while val != 0:
        ...
        val = main_a % main_b   if main_a > main_b   else main_b % main_a
        main_b = main_b if main_b>main_a else main_a
        main_a = val
        break

    return main_a if main_a>main_b else main_b
...
def reduce(f, xs, init=None):
    ...
    init = f(init,xs[start])
    if len(xs)==1:
        return init
    
    return reduce(f,xs[start+1:],init)
